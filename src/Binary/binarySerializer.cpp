...
void binary::BinarySerializer::serializeBase(::Meta::Meta* meta, binary::Meta& binaryMetaStruct)
{
    ...
    string topLevelModuleName = topLevelModule->getFullModuleName();
...
void binary::BinarySerializer::serializeBaseClass(::Meta::BaseClassMeta* meta, binary::BaseClassMeta& binaryMetaStruct)
{
    ...
    vector<MetaFileOffset> offsets;

    // instance methods
    sort(meta->instanceMethods.begin(), meta->instanceMethods.end(), compareMetasByJsName< ::Meta::MethodMeta>);
    ...
    // static methods
    sort(meta->staticMethods.begin(), meta->staticMethods.end(), compareMetasByJsName< ::Meta::MethodMeta>);
    ...
    // instance properties
    sort(meta->instanceProperties.begin(), meta->instanceProperties.end(), compareMetasByJsName< ::Meta::PropertyMeta>);
    ...
    // static properties
    sort(meta->staticProperties.begin(), meta->staticProperties.end(), compareMetasByJsName< ::Meta::PropertyMeta>);
    ...
    // protocols
    sort(meta->protocols.begin(), meta->protocols.end(), compareMetasByJsName< ::Meta::ProtocolMeta>);
    ...
    for (vector< ::Meta::MethodMeta*>::iterator it = meta->instanceMethods.begin(); it != meta->instanceMethods.end(); ++it) {
        if ((*it)->getFlags(::Meta::MetaFlags::MethodIsInitializer)) {
            firstInitializerIndex = (int16_t)distance(meta->instanceMethods.begin(), it);
            break;
        }
    }
...
void binary::BinarySerializer::serializeMethod(::Meta::MethodMeta* meta, binary::MethodMeta& binaryMetaStruct)
{
    ...
    binaryMetaStruct._constructorTokens = this->heapWriter.push_string(StringUtils::join(meta->constructorTokens, ":"));
}
...
void binary::BinarySerializer::serializeContainer(vector<pair<clang::Module*, vector< ::Meta::Meta*> > >& container)
{
    this->start(container);
    for (pair<clang::Module*, vector< ::Meta::Meta*> >& module : container) {
...
static llvm::ErrorOr<llvm::SmallString<128>> getFrameworkLib(clang::Module* framework, const string& library) {
    ...
    if (Expected<OwningBinary<Binary> > binaryOrErr = createBinary(path.get())) {
        ...
        if (MachOUniversalBinary* machoBinary = dyn_cast<MachOUniversalBinary>(&binary)) {
            for (const MachOUniversalBinary::ObjectForArch& object : machoBinary->objects()) {
                if (Expected<unique_ptr<MachOObjectFile> > objectFile = object.getAsObjectFile()) {
                ...
                } else if (Expected<unique_ptr<Archive> > archive = object.getAsArchive()) {
                    return true;
                }
            }
...
void binary::BinarySerializer::serializeModule(clang::Module* module, binary::ModuleMeta& binaryModule)
{
    ...
    vector<clang::Module::LinkLibrary> libraries;
    ::Meta::Utils::getAllLinkLibraries(module, libraries);
    vector<MetaFileOffset> librariesOffsets;
...
void binary::BinarySerializer::start(vector<pair<clang::Module*, vector< ::Meta::Meta*> > >& container)
{
}

void binary::BinarySerializer::finish(vector<pair<clang::Module*, vector< ::Meta::Meta*> > >& container)
{
}
