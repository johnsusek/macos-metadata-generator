...
binary::MetaFileOffset binary::BinaryTypeEncodingSerializer::visit(vector< ::Meta::Type*>& types)
{
    ...
    for (::Meta::Type* type : types) {
        ...
        binaryEncodings.push_back(move(binaryEncoding));
    }
...
unique_ptr<binary::TypeEncoding> binary::BinaryTypeEncodingSerializer::visitBridgedInterface(const ::Meta::BridgedInterfaceType& type)
{
    ...
    if (type.bridgedInterface == nullptr) {
        throw logic_error(string("Unresolved bridged interface for BridgedInterfaceType with name '") + type.name + "'.");
    }
    auto s = new binary::InterfaceDeclarationReferenceEncoding();
...
unique_ptr<binary::TypeEncoding> binary::BinaryTypeEncodingSerializer::serializeRecordEncoding(const binary::BinaryTypeEncodingType encodingType, const vector< ::Meta::RecordField>& fields)
{
